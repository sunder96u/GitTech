...
buildNewsFeed = async () => {
    ...
    const feed = await axios.get(`http://localhost:3001/api/post/`)
    newsFeed.innerHTML = ``
    let currentPost = []
    let currentComment = []
    let postComment = []
    ...
    for (let i = 0; i < feed.data.length; i++) {
        ...
        let postLikes = await axios.get(`http://localhost:3001/api/postLike/${feed.data[i]._id}`)
        ...
        for (let j = 0; j < comment.length; j++){
            comment[j].onclick = async () => {
                ...
                let postComments = await axios.get(`http://localhost:3001/api/comment/${currentPost[j]}`)
                for (let k = 0; k < postComments.data.length; k++) {
                    let commenterUser = await axios.get(`http://localhost:3001/api/user/${postComments.data[k].userId}`)
                ...
                input.onkeydown = async () => {
                    if (event.keyCode === 13) {
                        comment.innerHTML = ""
                        const newComment = await axios.post(`http://localhost:3001/api/comment?userId=${user.data._id}&date=${currentDate}&postId=${currentPost[j]}&comment=${input.value}`)
                    }
                }
            }
        }
        ...
        for (let j = 0; j < share.length; j++){
            share[j].onclick = () => {
                ...
                btn1.onclick = async () => {
                    ...
                    buildNewsFeed()
                    const sharePost = await axios.post(`http://localhost:3001/api/post/?userId=${user.data._id}&date=${currentDate}&text=${feed.data[j].text}`)
    ...
    modalContent.innerHTML = `
        ...
        input.onkeydown = async () => {
            if (event.keyCode === 13) {
                ...
                const createNewPost = await axios.post(`http://localhost:3001/api/post/?userId=${user.data._id}&date=${date}&text=${newPost}`)
            }
            buildNewsFeed()
        }
