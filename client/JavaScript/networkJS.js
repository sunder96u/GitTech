...
buildLeftSideBar = async () => {
    ...
    const myUser = await axios.get(`https://gittech-production.up.railway.app/api/user/${user}`)
    user = myUser
...
buildPendingInvites = async () => {
    ...
    const pendingconnection = await axios.get(`https://gittech-production.up.railway.app/api/connection/pending/${user}`)
    const pendingCon = await axios.get(`https://gittech-production.up.railway.app/api/connection/myinvites/${user.data._id}`)
    pending.innerHTML = ""
...
buildConnections = async () => {
    const connections = document.querySelector('.connections')
    const connection = await axios.get(`https://gittech-production.up.railway.app/api/connection/${user}`)
    const myConnections = await axios.get(`https://gittech-production.up.railway.app/api/connection/myconnections/${user.data._id}`)
...
buildInNetwork = async () => {
    ...
    const users = await axios.get(`https://gittech-production.up.railway.app/api/connection`)
    const connections = []
            ...
            for (let k = 0; k < connect.length; k++) {
                connect[k].onclick = async () => {
                    for (let l = 0; l < connections.length; l++) {
                        const connectTo = await axios.post(`https://gittech-production.up.railway.app/api/connection/add?userId=${user.data._id}&connection=${connections[k]}&accepted=true`)
                    }
                }
            }
        }
